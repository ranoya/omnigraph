play = function (p) {
  let id = windowmanager.createframe("", {
    ...
    "Atari 800": "Atari 800",
    "Apple II": "Apple II",
    Nintendo: "Nintendo Entreteinment System",
    Genesis: "Mega Drive",
    MSX: "MSX",
    PC: "DOS",
    Arcade: "Arcade",
  ...
  omnifilter = function (arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      code += `<div onclick="ifr('${arr[i].Name}', '${decodeURI(
        arr[i].Link,
      )}', false, '#000000');" style="width: 100%; aspect-ratio: 4/3; background-repeat: no-repeat; background-size: cover; background-position: center center; background-image: url(${
        arr[i].screenshots
      })"></div>`;
    }
